function [Blobs,Precision,Recall,F1Score,labels,scores,tp,fp,tn,fn,num_blobs_gt_out]=PeopleDetectionEval(GTBlobs,TestBlobs,num_frames)
...
for frame=1:num_frames
    ...
    [blob_asignados_gt,blob_asignados_test]=BlobsAssigment(GTBlobs{frame},TestBlobs{frame});
    
    num_blobs_asignados=size(blob_asignados_gt,2);
    ...
    for i=1:num_blobs_gt
        ...
        blob_aux.h=blob.h;
        blob_aux.score=-1;
        if(isempty(find(blob_asignados_gt==i)))%%blob no detectado
    ...
    for i=1:num_blobs_test
        ...
        if(~isempty(find(blob_asignados_test==i)))%%Blob acertado
